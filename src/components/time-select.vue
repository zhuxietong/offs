<template>
  <view v-if="modelValue" @click.stop="show">
    <slot :value="modelValue">
      <view style="color: var(--offs-color-dark4); font-size: 26rpx">{{ modelValue }} </view>
    </slot>
  </view>
  <view v-else @click.stop="show" style="font-size: 26rpx; color: var(--offs-color-dark4)"
    >选择时间
  </view>
    ...
    <view>
      ...
      >
        <me-tab
          ...
          :pill-style="{ radius: '10rpx', height: '80rpx', color: '#ffffff', padding: '0 10rpx' }"
          :active-style="{
            background: 'var(--offs-color-primary1)',
            fontSize: '26rpx',
            color: '#ffffff',
          }"
          :normal-style="{ background: 'transparent', fontSize: '26rpx', color: '#363636' }"
          field="label"
          v-model="dimIndex"
          pills
        ></me-tab>
      ...
      <view
        ...
        "
      >
        <view class="offs-btn normal mini" @click="close">关闭</view>
        <view class="offs-btn primary mini" @click="confirm">确定</view>
      </view>
    </view>
  </me-cmp-modal>
</template>
